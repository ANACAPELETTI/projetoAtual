...
public class IteratePSO {
	...
	public static void refreshPSO(List<PSOEntity> psoEntityList, List<ImageEntity> imageList, double omega) throws InterruptedException {
		...
		for (int particle = 0; particle < psoEntityList.size(); particle++) {
			...
			for(int i = 0; i < psoEntityList.get(particle).getVelocities().size(); i++) {
				for(int j = 0; j < psoEntityList.get(particle).getVelocities().get(i).size(); j++) {
					PSOUtils.atualizaVelocidade(
							...
							psoEntityList.get(particle).getKernels().get(i).get(j), 
							omega);
				}
			...
			//psoEntityList.get(particle).setFullyConectedLayerParticle(PSOUtils.newConnected(psoEntityList.get(particle).getFullyConectedLayerParticle(), melhorGlobal.getFullyConectedLayerParticle(),omega));
			psoEntityList.get(particle).setFullyConectedLayerParticle(PSOUtils.newConnected(psoEntityList.get(particle).getFullyConectedLayerParticle(), psoEntityList.get(particle).getFullyConectedVelocity(),psoEntityList.get(particle).isMelhorGlobal()));
			...
			int lastLayerSize = psoEntityList.get(particle).getLayers().get(psoEntityList.get(particle).getLayers().size() - 2).size();
			...
			for (int i = 0; i < firstLayerSize; i++) {
				...
				for (int j = 0; j < nClassificacoes; j++) {
					...
					fullyList.add( psoEntityList.get(particle).getLayers().get(psoEntityList.get(particle).getLayers().size() - 2)
			...
			for (int i = 0; i < finalLayer.size(); i++) {
				listFullyConnectedLayer
						.add(FullyConected.fullyConected(finalLayer.get(i), psoEntityList.get(particle).getFullyConectedLayerParticle()));
			...
			if(sumErros < psoEntityList.get(particle).getErroGlobal()) {
				...
				psoEntityList.get(particle).setFullyConectedMelhorLocal(psoEntityList.get(particle).getFullyConectedLayerParticle());
			}
